...
use assignsubmission_ctsubmission\helper as CTHELPER;
use \availability_ctcompletion\shared_lib AS NED;
...
class frontend extends \core_availability\frontend {
    ...
    protected function get_javascript_init_params($course, \cm_info $cm = null, \section_info $section = null) {
        ...
        if ($cachekey !== $this->cachekey) {
            ...
            if ($activities = CTHELPER::get_ctsubmission_assignments($course->id)) {
                foreach ($activities as $activity) {
                    $ctsubmissionenabled[] = $activity->assignment;
                }
                foreach ($modinfo->cms as $id => $othercm) {
                    // Add each course-module if it has completion turned on and is not
                    // the one currently being edited.
                    if ($othercm->completion && (empty($cm) || $cm->id != $id) && !$othercm->deletioninprogress && in_array($othercm->instance, $ctsubmissionenabled)) {
                        $cms[] = (object)['id' => $id,
                ...
                if ($previouscm) {
                    $previous = (object)['id' => \availability_ctcompletion\condition::OPTION_PREVIOUS,
                        'name' => get_string('option_previous', 'availability_ctcompletion'),
                        'completiongradeitemnumber' => \availability_ctcompletion\condition::OPTION_PREVIOUS];
