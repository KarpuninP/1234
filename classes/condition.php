 ...
 *
 * @package availability_ctcompletion
 * @copyright 2014 The Open University
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

namespace availability_ctcompletion;
...
class condition extends \core_availability\condition {
    ...
    /** @var int CT Completion */
    public const CT_COMPLETION = 4;
    ...
    public function is_available($not, info $info, $grabthelot, $userid): bool {
        ...
        if (!array_key_exists($cmid, $modinfo->cms) || $modinfo->cms[$cmid]->deletioninprogress) {
            ...
            if ($this->expectedcompletion == self::CT_COMPLETION) {
                $ctcompletionstatuses = [COMPLETION_COMPLETE, COMPLETION_COMPLETE_PASS];
                $context = \context_course::instance($info->get_course()->id);
                $allow = false;

                if ($usergroups = groups_get_user_groups($info->get_course()->id)) {
                    foreach ($usergroups as $groupingid => $ggroups) {
                        foreach ($ggroups as $groupid) {
                            if ($cts = get_enrolled_users($context, 'availability/ctcompletion:completeforgroup', $groupid, 'u.id', null, 0, 0, true)) {
                                foreach ($cts as $ct) {
                                    $completiondata = $completion->get_data((object)['id' => $cmid], $grabthelot, $ct->id);
                                    if (in_array($completiondata->completionstate, $ctcompletionstatuses)) {
                                        $allow = true;
                                        break 2;
                                    }
                                }
    ...
    private function load_course_structure(stdClass $course): void {
        ...
        if (empty($this->modfastprevious)) {
            $previouscache = cache::make('availability_ctcompletion', 'ct_previous_cache');
    ...
    public function get_description($full, $not, info $info): string {
        ...
        if ($this->cmid == self::OPTION_PREVIOUS && $editing) {
            ...
            $modinfo = $info->get_modinfo();
            if (!array_key_exists($cmid, $modinfo->cms) || $modinfo->cms[$cmid]->deletioninprogress) {
                $modname = get_string('missing', 'availability_ctcompletion');
            } else {
        ...
        return get_string($str, 'availability_ctcompletion', $modname);
    }
    ...
    public static function completion_value_used($course, $cmid): bool {
        ...
        if (!array_key_exists($course->id, self::$modsusedincondition)) {
            ...
            foreach ($modinfo->cms as $othercm) {
                ...
                foreach ($tree->get_all_children('availability_ctcompletion\condition') as $cond) {
